...
impl Resource {
    ...
    pub fn remaining_capacity(&self) -> u32 {
        self.target_size.saturating_sub(self.total_saved_size)
    }
...
impl InputBuffer {
    ...
    // 处理生产者事件
    pub fn handle_producer_event(
        &mut self,
        event: &ProducerEvent,
    ) -> anyhow::Result<ProducerEventResult> {
...
impl ResultBuffer {
    ...
    // 处理消费者事件
    pub fn handle_consumer_event(
        &mut self,
        event: &ConsumerEvent,
    ) -> anyhow::Result<ConsumerEventResult> {
        if let Some(ref mut destination) = self.destination {
            let consume_length =
                std::cmp::min(event.maximum_consume_length, destination.current_size);
...
impl BufferPair {
    ...
    pub fn handle_buffer_event(&mut self, event: BufferEvent) -> anyhow::Result<BufferEventResult> {
        debug!(
            "[BufferPair] Processing buffer event: {:?} for owner: {:?}",
            event, self.owner
        );
        ...
        let result = match event {
            BufferEvent::Producer(producer_event) => {
                ...
                debug!(
                    "[FORWARD-INFO] Resource index: {}, append length: {} bytes",
                ...
                debug!(
                    "[FORWARD-INFO] Producer result: accepted={} bytes, remaining={} bytes",
            ...
            BufferEvent::Consumer(consumer_event) => {
                ...
                debug!("[FORWARD-INFO] Owner: {:?}", self.owner);
                debug!(
                    "[FORWARD-INFO] Maximum consume length: {} bytes",
                ...
                debug!(
                    "[FORWARD-INFO] Consumer result: consumed={} bytes, remaining={} bytes",
    ...
    pub fn debug_status(&self) {
        ...
        for (i, resource) in self.input_buffer.resource.iter().enumerate() {
            ...
            debug!(
                "[BufferPair]   Resource[{}]: Type={}, Progress={}/{} bytes ({:.2}%)",
                i,
                resource_type,
    ...
    pub fn increase_result(&mut self, new_bytes: u32) -> anyhow::Result<()> {
        debug!(
            "[BufferPair] Increasing result buffer by {} bytes for {:?}",
            new_bytes, self.owner
        );
    ...
    pub(crate) fn get_memory_input_current_bytes(&self) -> anyhow::Result<u32> {
        if self
            ...
            .resource
            .iter()
            .any(|r| r.resource_type != ResourceType::Memory && !r.is_full())
        {
            return Ok(0);
        }
    ...
    pub(crate) fn set_input(&mut self, resource: Vec<Resource>) -> anyhow::Result<()> {
        debug!(
            "[BufferPair] Setting input buffer for {:?} with {} resources",
            self.owner,
            resource.len()
        );
        ...
        for (i, res) in resource.iter().enumerate() {
            ...
            debug!(
                "[BufferPair]   Resource[{}]: Type={}, Target size={} bytes",
    ...
    pub(crate) fn set_output(&mut self, destination: EnhancedResource, pc: usize) {
        ...
        debug!(
            "[BufferPair] Setting output buffer for {:?}: Type={}, Target size={} bytes",
    ...
    pub fn get_current_input_bytes(&self) -> anyhow::Result<u32> {
        if self.input_buffer.resource.iter().any(|r| {
            matches!(
                r.resource_type,
                ResourceType::Register(RegisterType::VectorRegister(_))
            )
        }) {
            Ok(self
                .input_buffer
...
impl EnhancedResource {
    ...
    pub fn remaining_capacity(&self) -> u32 {
        self.target_size.saturating_sub(self.current_size)
    }
