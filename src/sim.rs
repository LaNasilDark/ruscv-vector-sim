...
use unit::function_unit::{
    CommonFunctionUnit, FunctionUnitKeyType, FunctionUnitType, VectorFunctionUnit,
};
...
impl Simulator {
    pub fn new() -> Simulator {
        ...
        let _bytes_per_event = 8; // not used for now

        let vector_bytes_per_event = SimulatorConfig::get_global_config()
            .unwrap()
            .get_data_length();
        // 为模拟器注入功能单元
        // Vector类型使用VectorFunctionUnit
        function_units.insert(
            FunctionUnitKeyType::VectorAlu,
            FunctionUnitType::Vector(Box::new(VectorFunctionUnit::new(
                vector_bytes_per_event,
                FunctionUnitKeyType::VectorAlu,
            ))),
        );
        function_units.insert(
        ...
        function_units.insert(
            ...
            FunctionUnitType::Common(CommonFunctionUnit::new(FunctionUnitKeyType::FloatAlu)),
        );
        function_units.insert(
            FunctionUnitKeyType::FloatMul,
            FunctionUnitType::Common(CommonFunctionUnit::new(FunctionUnitKeyType::FloatMul)),
        );
        function_units.insert(
            FunctionUnitKeyType::FloatDiv,
            FunctionUnitType::Common(CommonFunctionUnit::new(FunctionUnitKeyType::FloatDiv)),
        );
        function_units.insert(
            FunctionUnitKeyType::IntegerAlu,
            FunctionUnitType::Common(CommonFunctionUnit::new(FunctionUnitKeyType::IntegerAlu)),
        );
        function_units.insert(
            FunctionUnitKeyType::IntergerDiv,
            FunctionUnitType::Common(CommonFunctionUnit::new(FunctionUnitKeyType::IntergerDiv)),
        );
    ...
    fn handle_buffer_event(&mut self, key: UnitKeyType, event: BufferEvent) -> BufferEventResult {
        ...
        let result = match key {
            UnitKeyType::FuncKey(func_key) => {
                ...
                match fu {
                    FunctionUnitType::Vector(fu) => fu.handle_buffer_event(event),
                    _ => unreachable!("Common Function Unit does not have buffer"),
                }
            }
            UnitKeyType::MemKey(mem_key) => self.memory_unit.handle_buffer_event(mem_key, event),
        };
    ...
    fn handle_register_file(&mut self) {
        ...
        register_file.iter_mut_tasks().for_each(|r| {
            if r.task_queue().is_empty() {
                //如果当前寄存器没有待处理任务，直接跳过
                return;
            }
            debug!(
                "Processing register task queue, length: {}",
                r.task_queue().len()
            );
            ...
            while let Some(e) = r.generate_event() {
                ...
                debug!(
                    "Generated register event: {:?}, target unit: {:?}",
                    e, unit_key
                );
                ...
                FunctionUnitType::Common(fu) => {
                    ...
                    match res {
                        ...
                        CommonEventResult::WriteResultTo(reg) => {
                            debug!(
                                "[{:?}] Function unit {:?} write result to register {:?}",
                                key, key, reg
                            );
        ...
        if let Some(inst) = self.fetch_unit.fetch() {
            ...
            match inst {
                Inst::Func(func_inst) => {
                    ...
                    let fu = self
                        .function_unit
                        .get_mut(&func_inst.func_unit_key)
                        .unwrap();
                    match fu {
                        FunctionUnitType::Common(fu) => {
                            // 检查功能单元是否可以接受新指令
                            let can_issue_register = self
                                .register_file
                                .borrow()
                                .can_issue_common_instruction(&func_inst);

                            debug!(
                                "Issue check for {:?}: register_file_ready={}",
                        ...
                        FunctionUnitType::Vector(fu) => {
                            ...
                            if self
                                .register_file
                                .borrow()
                                .can_issue_vector_instruction(&func_inst)
                                && fu.can_accept_new_instruction()
                            {
                                fu.issue(func_inst.clone(), self.fetch_unit.get_pc())?;
                                self.fetch_unit.next_pc();
                                ...
                                debug!(
                                    "Function unit {:?} cannot accept new instruction yet, waiting",
                                    func_inst.func_unit_key
                                );
                            }
                        }
                    }
                ...
                Inst::Mem(mem_inst) => {
                    ...
                    if self.memory_unit.can_accept_new_instruction(mem_inst.dir)
                        && self
                            .register_file
                            .borrow()
                            .can_issue_memory_instruction(&mem_inst)
                    {
                        debug!("Memory unit can accept new instruction, issuing memory instruction: {:?}", mem_inst);

                        // issue 这条指令
                        let port_index =
                            self.memory_unit.issue(mem_inst, self.fetch_unit.get_pc())?;

                        // 添加对寄存器文件的任务
                        self.register_file
                            .borrow_mut()
                            .add_mem_task(&mem_inst, port_index);
                        // 更新PC值，这行是新增的
                        self.fetch_unit.next_pc();
                        // 处理完Inst::Mem后再次添加
                        // self.memory_unit.debug_port_status();
                        ...
                        debug!(
                            "Memory unit issued instruction: {:?} at cycle {}, PC advanced",
                            mem_inst.raw, current_cycle
                        ); //TODO: update lifecycle info
                    } else {
    ...
    pub fn main_sim_loop(&mut self) -> anyhow::Result<()> {
        ...
        while !self.is_simulation_end() && total_cycle < max_cycles {
            ...
            info!(
                "Step 1.3: Increasing consumed bytes for memory-type in write port result buffer"
            );
            ...
            total_cycle += 1;
            info!(
                "========== Simulation for cycle {} completed ==========",
                total_cycle - 1
            );
        }
        ...
        if total_cycle >= max_cycles {
            debug!(
                "Reached maximum cycle limit ({}), simulation stopped",
                max_cycles
            );
        } else {
