...
impl Display for Instruction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Instruction::VREDSUM_VS { vrd, vrs1, vrs2 } => write!(f, "vredsum.vs v{vrd}, v{vrs2}, v{vrs1}"),
            Instruction::VFSLIDE1DOWN_VF { vrd, frs1, vrs2 } => write!(f, "vfslide1down.vf v{vrd},  v{vrs2}, {frs1}"),
            Instruction::VFSLIDE1UP_VF { vrd, frs1, vrs2 } => write!(f, "vfslide1up.vf v{vrd},  v{vrs2}, {frs1}"),
            Instruction::VMV_V_V { vrd, vrs1 } => write!(f, "vmv.v.v v{vrd}, v{vrs1}"),
            Instruction::VMVnR_V { vrd, vrs1 } => write!(f, "vmvnr.v v{vrd}, v{vrs1}"),
            Instruction::VSETIVLI { rd, uimm, sew, lmul, tail, mask } => write!(f, "vsetivli x{rd}, {uimm}, e{sew}, {lmul}, t{tail}, m{mask}"),
            Instruction::VSETVL { rd, rs1, rs2 } => write!(f, "vsetvli x{rd}, x{rs1}, x{rs2}"),
            Instruction::VSETVLI { rd, rs1, sew , lmul , tail , mask } => write!(f, "vsetvli x{rd}, x{rs1}, e{sew}, {lmul}, t{tail}, m{mask}"),
            Instruction::VFMACC_VV { vrd, vrs1, vrs2 } => write!(f, "vmacc.vv v{vrd}, v{vrs1}, v{vrs2}"),
