...
use crate::instruction::{Instruction, Iorw, VectorOption, LMUL};
...
pub fn decode(code: u32, target: &Target) -> Instruction {
    ...
    debug!("code {:08x}", code);
    debug!("opcode {:?}, funct3 {:?}, funct7 {:?}", opcode, funct3, funct7);
    // Mega decoding table!
    //
    ...
    match (opcode, funct3, funct7) {
        ...
        // Vector Extension
        (Op::LOAD_FP, _ , _) => {
            // TODO: handle vector indexed load/ strided load
            match funct3 {
                0b000 => VLE { vrd: rd , rs1, width : 8},
                0b101 => VLE { vrd: rd , rs1, width : 16},
                0b110 => VLE { vrd: rd , rs1, width : 32},
                0b111 => VLE { vrd: rd , rs1, width : 64},
                _ => panic!("Invalid WIDTH for Vector Load"),
            }
        },
        (Op::STORE_FP, _, _) => {
            // TODO: handle vector indexed load/ strided store
            match funct3 {
                0b000 => VSE { vrd: rd, rs1, width : 8},
                0b101 => VSE { vrd: rd, rs1, width : 16 },
                0b110 => VSE { vrd: rd, rs1, width : 32 },
                0b111 => VSE { vrd: rd, rs1, width : 64 },
                _ => panic!("Invalid WIDTH for Vector Store"),
            }
        },
        ...
        (Op::OPV, OPV::OPFVV, _) => {
            match funct6 {
                0b000000 => VFADD_VV { vrd: rd, vrs1: rs1, vrs2: rs2 },
                0b100100 => VFMUL_VV { vrd: rd, vrs1: rs1, vrs2: rs2 },
                0b101100 => VFMACC_VV { vrd: rd, vrs1: rs1, vrs2: rs2 },
                _ => unimplemented!()
            }
        },
        (Op::OPV, OPV::OPMVV, _) => {
            match funct6 {
                0b000000 => VREDSUM_VS { vrd: rd, vrs1: rs1, vrs2: rs2 },
        ...
        (Op::OPV, OPV::OPIVI, _) => {
            match funct6 {
                0b100111 => VMVnR_V { vrd: rd, vrs1: rs1 },
                _ => unimplemented!()
            }
        },
        (Op::OPV, OPV::OPIVX, _) => {
            match funct6 {
                0b000000 => VADD_VX { vrd: rd, rs1, vrs2: rs2 },
                _ => unimplemented!()
            }
        },
        (Op::OPV, OPV::OPFVF, _) => {
            match funct6 {
                0b001110 => VFSLIDE1UP_VF { vrd: rd, frs1: rs1, vrs2: rs2 },
                0b001111 => VFSLIDE1DOWN_VF { vrd: rd, frs1: rs1, vrs2: rs2 },
                _ => unimplemented!()
            }
        },
        (Op::OPV, OPV::OPMVX, _) => {
...
pub struct Encoding {
    ...
    pub funct7: u32,
    /// 6-bit opcode extension present in V-extensions
    pub funct6 : u32,
