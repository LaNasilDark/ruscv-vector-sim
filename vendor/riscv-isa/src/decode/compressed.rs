...
pub fn decode(code: u16, target: &Target) -> Compressed {
    ...
    debug!("Decoding compressed instruction: code=0x{:04x}, opcode={:02b}, funct3={:03b}", code, opcode, funct3);
    ...
    match opcode {
        0b00 => {
            ...
            debug!("Compressed opcode=00: rd={}, rs1={}, rs2={}", rd, rs1, rs2);
            match funct3 {
                _ if code == 0 => UNIMP,
        ...
        0b01 => {
            ...
            let z = (code >> 12) & 0b1;
            debug!("Compressed opcode=01: rd={}, rd_p={}, rs1={}, rs2={}, x={:05b}, y={:05b}, z={:01b}", rd, rd_p, rs1, rs2, x, y, z);
            match (funct3, z, y, x) {
                ...
                (0b011, _, _, _) if ext_zca => C_LUI { rd, imm: imm_lui(code) },
                (0b100, 0, y, _) if ext_zca && y >> 3 == 0b00 => {
                    debug!("Decoding C_SRLI: rd={}, shamt={}, y={:05b}, y>>3={:02b}", rd_p, imm_li(code) as u32, y, y >> 3);
                    C_SRLI { rd: rd_p, shamt: imm_li(code) as u32 }
                },
                (0b100, 1, y, _) if ext_zca && y >> 3 == 0b00 && rv64 => {
                    debug!("Decoding C_SRLI: rd={}, y={:05b}, y>>3={:02b}, rv64: {rv64}, shamt: {}", rd_p,  y, y >> 3, imm_li_rv64(code));
                    C_SRLI { rd: rd_p, shamt: imm_li_rv64(code) as u32 }
                }
                (0b100, 0, y, _) if ext_zca && y >> 3 == 0b01 => {
                    debug!("Decoding C_SRAI: rd={}, shamt={}, y={:05b}, y>>3={:02b}", rd_p, imm_li(code) as u32, y, y >> 3);
        ...
        0b10 => {
            ...
            let z = (code >> 12) & 0b1;
            match (funct3, z, y, x) {
                (0b000, 0, _, _) if ext_zca => C_SLLI { rd, shamt: imm_li(code) as u32 },
...
fn imm_li_rv64(code: u16) -> i32 {
    let code = code as i32;
    (((code >> 12) & 0b1) << 5) | ((code >> 2) & 0b11111)
}
