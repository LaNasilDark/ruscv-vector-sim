...
extern void matrixmul_intrinsics_tiled(data_t *a, data_t *b, data_t *c, int n, int m, int p) ;
...
int main (int argc, char **argv)
{
    ...
    printf("Read Matrix B ends\n");

    // Read Matrix Reference
    fgets(line, sizeof(line), file);  // Read the blank line
    read_vector(file, reference, M*N, N);
    printf("Read Matrix Reference Ends\n");
    ...
    // matrixmul_intrinsics(M1, M2_T, result, N, M, K);
    // printf("matrixmul_intrinsics done\n");
    
    // 使用tiled matrix multiply
    matrixmul_intrinsics_tiled(M1, M2_T, result, N, M, K);
    printf("matrixmul_intrinsics done\n");
