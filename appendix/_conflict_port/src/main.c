...
void vector_f64_operation(double *a1, double *a2, double *a3, double *result) {
    ...
    // VADD.v V5,V3,V4
    vfloat64m1_t v5 = vfadd_vv_f64m1(v1, v3, gvl);
...
int main() {
    srand(time(NULL));
    double a1[ARRAY_SIZE] __attribute__((aligned(16)));
    double a2[ARRAY_SIZE] __attribute__((aligned(16)));
    double a3[ARRAY_SIZE] __attribute__((aligned(16)));
    ...
    printf("The address of result %#x\n", result);
    // 打印结果
    for (int i = 0; i < ARRAY_SIZE; i++) {
        printf("Result[%d]: %lf\n", i, result[i]);
    }
